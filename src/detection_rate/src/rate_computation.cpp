...
void doMsg(const detection_rate::Sensor::ConstPtr &msgp1, const detection_rate::Sensor::ConstPtr &msgp2)
{    
    int msgp1_data = atoi(msgp1->data.c_str());
    int msgp2_data = atoi(msgp2->data.c_str());
    // //ROS_INFO("%s", msgp->data.c_str());
    // ROS_INFO("%d%d", msgp1_data, msgp2_data);
    total_time++;
    if (msgp1_data == 1 || msgp2_data == 1)
    {
        successful_time++;
    }
...
int main(int argc, char *argv[])
{
    ...
    TimeSynchronizer<detection_rate::Sensor, detection_rate::Sensor> sync(sensor1_sub, sensor2_sub, 10);    
    sync.registerCallback(boost::bind(&doMsg, _1, _2));
