...
idx_helix = optimizableVariable('idx_helix1',[1,size_edge_candidate],'Type','integer');
idx_link = optimizableVariable('idx_link1',[2,7],'Type','integer');
size_spot = optimizableVariable('size_spot1',[8,16],'Type','integer');

fun = @(x)detective_rate_helix(x.idx_helix1,x.size_spot1,r_obj_offset,theta_obj_offset,target_homo,joint_constraint,a,d,alpha,edge_candidate,point_all,edge, r_cylinder,size_point_target,fov_vertical,fov_horizontal,size_sim,h_cone,x.idx_link1);

results = bayesopt(fun,[idx_helix,idx_link,size_spot],'UseParallel',true);