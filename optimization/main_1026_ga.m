...
% detective_rate = getDetectiveRate_single(1, 9943, 16, size_point_target, size_sim, r_obj_offset, theta_obj_offset, target_homo, q_all, my_robot, size_joint, all_tform_spot, all_tform_spot_link0, all_tform_spot_link1, all_tform_spot_link2, all_tform_spot_link3, all_tform_spot_link4, all_tform_spot_link5, all_tform_spot_link6, fov_vertical, fov_horizontal, h_cone);
%%%%%%%%%%%%%%%
%%% 优化参数 %%%
%%%%%%%%%%%%%%%
...
% 使用所有ring
% fun = @(x)detective_rate_franka(x.idx_geodesic0,x.idx_geodesic1,x.idx_geodesic2,x.idx_geodesic3,x.idx_geodesic4,x.idx_geodesic5,x.idx_geodesic6,size_sim, r_obj_offset, theta_obj_offset, target_homo, q_all, my_robot, size_joint, X_link, F_link, edge_candidate_all1, a, d, alpha, size_spot, size_point_target, fov_vertical, fov_horizontal, h_cone);
% results = bayesopt(fun,[idx_geodesic0,idx_geodesic1,idx_geodesic2,idx_geodesic3,idx_geodesic4,idx_geodesic5,idx_geodesic6],'UseParallel',true,'GPActiveSetSize',300,'ExplorationRatio',0.5,'ParallelMethod','clipped-model-prediction','MaxObjectiveEvaluations',30,'NumSeedPoints',4);
% 优化ring的数量
