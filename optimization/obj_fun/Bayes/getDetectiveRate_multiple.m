function detective_rate = getDetectiveRate_multiple(size_sensor, idx1, idx2, idx3, idx4, idx5, idx6, idx7, idx_geodesic0, idx_geodesic1, idx_geodesic2, idx_geodesic3, idx_geodesic4, idx_geodesic5, idx_geodesic6, size_spot, size_point_target, size_sim, r_obj_offset, theta_obj_offset, target_homo, q_all, my_robot, size_joint, all_tform_spot_link0, all_tform_spot_link1, all_tform_spot_link2, all_tform_spot_link3, all_tform_spot_link4, all_tform_spot_link5, all_tform_spot_link6, fov_vertical, fov_horizontal, h_cone)
idx_link = [idx1, idx2, idx3, idx4, idx5, idx6, idx7];
...
parfor idx_config = 1:size_sim
    ...
    % 对各连杆上的传感器进行配置
    for idx_round = 1:size_sensor % 遍历所有连杆
        ...
        tform_spot = all_tform_spot{1,idx_geodesic(idx_link(idx_round)+1)};
...
detective_rate = -detection_times/size_sim + 9999*~(numel(unique(idx_link(1:size_sensor))) == size_sensor);
end