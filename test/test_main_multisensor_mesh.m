...
% for i = 1:size_joint
%     q_all(:,i) = (joint_constraint(1,i).*ones(1,size_sim) + (joint_constraint(2,i) - joint_constraint(1,i)).*rand(1,size_sim))';
% end
...
for idx_config = 1:size_sim
    ...
    link_name = ["panda_link0_mesh" "panda_link1_mesh" "panda_link2_mesh" "panda_link3_mesh" "panda_link4_mesh" "panda_link5_mesh" "panda_link6_mesh" "panda_link7_mesh"];
    config = homeConfiguration(my_robot); % 关节空间配置结构体生成
    for i = 1:size_joint % 遍历每个关节
        config(i).JointPosition = q(1,i);
    end

    TR0 = stlread('C:\projects\MATLAB\robot_sensor\franka_description\meshes\visual\link2.stl');
    X0 = TR0.Points';
    X0 = [X0;ones(1,size(X0,2))];
    F0 = TR0.ConnectivityList;
    transform = getTransform(my_robot,config,'panda_link2');
    X1 = transform*X0;
    TR1 = triangulation(F0,X1(1:3,:)');
    figure(1);
    % trimesh(TR0);
    % hold on
    trimesh(TR1);
    ...
    for idx_link = 1:size_joint % 遍历所有连杆
        link_current = findobj('DisplayName',link_name(idx_link)); % 搜索当前连杆mesh信息
        F = (double(link_current.Faces))'; % 当前连杆mesh的三角面片信息
        size_F = size(F,2); % 三角面数量
        X = (double(link_current.Vertices))'; % 当前连杆mesh的顶点坐标信息
        ...
        interval = (max(paths(1,:))-min(paths(1,:)))/size_interval; % 拟合路径点间距
        x = min(paths(1,:)):interval:max(paths(1,:)); % 拟合路径点的x坐标

        coefficient_y = polyfit(paths(1,:),paths(2,:),3); % y-x三次多项式曲线拟合系数
        y = polyval(coefficient_y,x); % 拟合路径点的y坐标
        coefficient_z = polyfit(paths(1,:),paths(3,:),3); % z-x三次多项式曲线拟合系数
        z = polyval(coefficient_z,x); % 拟合路径点的z坐标
        ...
        % plot3(point_path(1,:),point_path(2,:),point_path(3,:)) % 绘制拟合路径
