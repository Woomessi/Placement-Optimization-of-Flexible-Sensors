...
% 选择起点
index_facet_current = 4631;% 初始化当前三角面片索引
barycentric1 = 0.2;
barycentric2 = 0.3;
barycentric3 = 1 - barycentric1 - barycentric2;
point_current_barycentric = [barycentric1,barycentric2,barycentric3];
point_current = barycentricToCartesian(TR,index_facet_current,point_current_barycentric);
...
while length_sum < length_desired
    ...
    % 排除连杆连接面的顶点（思考如何利用长度约束进行排除）
    if round >1
        if dot(faceNormal(TR,indices_facets_seq(round)),faceNormal(TR,indices_facets_seq(round-1)))<=0.01
            break
        end
    end
