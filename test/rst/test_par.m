...
point_all_mat = zeros(size_point,2); %储存所有离散点圆柱坐标的数组
...
length_graph1 = zeros(size_edge,1); %储存两离散点间测地线长度的数组
length_graph2 = zeros(size_edge,1); %储存两离散点间测地线长度的数组

edge1 = edge(:,1);
edge2 = edge(:,2);
point_all_mat11 = point_all_mat(edge(:,1),1);
point_all_mat12 = point_all_mat(edge(:,1),2);
point_all_mat21 = point_all_mat(edge(:,2),1);
point_all_mat22 = point_all_mat(edge(:,2),2);

parfor i = 1:size_edge
    ...
    % theta1 = point_all_mat(edge(i,1),1); %起点角度圆柱坐标
    % z1 = point_all_mat(edge(i,1),2); %起点高度圆柱坐标
    % theta2 = point_all_mat(edge(i,2),1); %终点角度圆柱坐标
    % z2 = point_all_mat(edge(i,2),2); %终点高度圆柱坐标

    theta1 = point_all_mat11(i); %起点角度圆柱坐标
    z1 = point_all_mat12(i); %起点高度圆柱坐标
    theta2 = point_all_mat21(i); %终点角度圆柱坐标
    z2 = point_all_mat22(i); %终点高度圆柱坐标

    % length_graph(i,1) = sqrt(r_cylinder^2*(theta2-theta1)^2+(z2-z1)^2); %测地线长度计算
    length_graph1(i) = sqrt(r_cylinder^2*(theta2-theta1)^2+(z2-z1)^2); %测地线长度计算
%共轭螺旋线
    xi1 = theta1+2*pi;  xi2 = theta2;
    % length_graph(i,2) = sqrt(r_cylinder^2*(xi2-xi1)^2+(z2-z1)^2);
    length_graph2(i) = sqrt(r_cylinder^2*(xi2-xi1)^2+(z2-z1)^2);
end
% toc
...
alternative_edge1 = zeros(size_edge,1); %储存满足长度条件的待选离散点对的数组
alternative_edge2 = zeros(size_edge,1); %储存满足长度条件的待选离散点对的数组

tol = 0.01; %长度容限
parfor i = 1:size_edge
    % if abs(length_graph(i,1) - l_sensor) < tol %测地线长度是否接近柔性传感器环长度
    if abs(length_graph1(i) - l_sensor) < tol %测地线长度是否接近柔性传感器环长度
        % alternative_edge(i,1) = i; %保存离散点对索引
        alternative_edge1(i) = i; %保存离散点对索引
    else
        % if abs(length_graph(i,2)-l_sensor) < tol %共轭测地线长度是否接近柔性传感器环长度
        if abs(length_graph2(i)-l_sensor) < tol %共轭测地线长度是否接近柔性传感器环长度
            % alternative_edge(i,2) = i; %保存离散点对索引
            alternative_edge2(i) = i; %保存离散点对索引
        end
    end
end
...
alternative_edge1(all(alternative_edge1==0,2),:)=[]; %清除数组全0行
size_alternative_edge1 = size(alternative_edge1,1); %待选离散点对总数
alternative_edge2(all(alternative_edge2==0,2),:)=[]; %清除数组全0行
size_alternative_edge2 = size(alternative_edge2,1); %待选离散点对总数
%%%%%%%%%%%%
%%% 作图 %%%
%%%%%%%%%%%
%{
...
parfor idx_joint = 1:size(path,2)%遍历每个关节
...
%%%%%%%%%%%%%%%%%%%%%
idx_geodesic = 30;
% if alternative_edge1(idx_geodesic)==0 %共轭测地线
if size_alternative_edge1 < idx_geodesic %共轭测地线
    idx = alternative_edge2(idx_geodesic);
    ...
    theta1 = point_all_mat(edge(idx,1),1); %起点角度圆柱坐标
    z1 = point_all_mat(edge(idx,1),2); %起点高度圆柱坐标
    theta2 = point_all_mat(edge(idx,2),1); %终点角度圆柱坐标
    z2 = point_all_mat(edge(idx,2),2); %终点高度圆柱坐标
    ...
    idx = alternative_edge1(idx_geodesic); %测地线
    % theta1 = point_all{edge(idx ,1)}(1);
    % z1 = point_all{edge(idx ,1)}(2);
    % theta2 = point_all{edge(idx ,2)}(1);
    % z2 = point_all{edge(idx ,2)}(2);
...
grid on
hold on
position_helix1 = position_helix(1,:);
position_helix2 = position_helix(2,:);
position_helix3 = position_helix(3,:);
spot1 = spot(1,:);
spot2 = spot(2,:);
spot3 = spot(3,:);
...
for i = 1:size_sensor
    ...
    % 圆柱螺旋线可视化
    plot3(position_helix1,position_helix2,position_helix3,'linewidth',1);
    % 均分点可视化
    scatter3(spot1,spot2,spot3,9,"magenta","filled")
    % 传感器坐标系可视化
    % for j = 1:size_sensor
