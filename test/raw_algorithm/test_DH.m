...
for idx_config = 1:size_sim %当前关节角配置
    ...
    q = q_all(idx_config,:);
    %坐标变换
    % tform_link = getTransform(my_robot, config, "panda_link7", "panda_link1");%在当前config下，某Joint_Link坐标系到基底坐标系的齐次变换矩阵
    tform_link = eye(4);
    for i = 1:idx_link
        % tform_link = tform_link*getTformDH(a(i),d(i),alpha(i),q(i));
