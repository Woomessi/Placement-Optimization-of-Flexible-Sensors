...
r_obj_offset = 0.42+(0.65-0.42)*rand(size_sim,1); % （单位：米）相对于机器人的距离随机偏移量
...
for idx_config = 1:size_sim
    ...
    % 对各连杆上的传感器进行配置
    for idx_link = 1:size_joint % 遍历所有连杆 (idx_link = 1 对应 link0，末端连杆对应空矩阵)
        ...
        % 选择当前连杆的传感器布局方案
        tform_spot = all_tform_spot{1,10};
