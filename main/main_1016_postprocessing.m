...
all_points_seq = all_points_seq(1,1:index_geodesic-1);

%%%%%%%%%%%%%%%%%
%%% 均分点提取 %%%
%%%%%%%%%%%%%%%%%
points_seq_current = all_points_seq{1,2000};
...
while spot(:,size_sensor-1) == zeros(3,1)
    ...
    if length1 > interval1
        t = 1-sqrt((norm(vector1)-length1+interval1).^2*((p1(1)-p2(1)).^2+(p1(2)-p2(2)).^2+(p1(3)-p2(3)).^2))./((p1(1)-p2(1)).^2+(p1(2)-p2(2)).^2+(p1(3)-p2(3)).^2);
        spot(:,j) = t*p1+(1-t)*p2;
        ...
        j = j+1;
        flag = 1;
        length1_sum = length1_sum + interval1;
    else
        i = i+1;
    end
end
...
tform_spot_all = cell(1,size_sensor);
for i = 1:size_sensor
    tform_spot_all{1,i} = [x_point_path(:,i),y_point_path(:,i),z_point_path(:,i),spot(:,i);0,0,0,1];
end
