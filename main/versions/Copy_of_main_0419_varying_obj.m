...
% plotHelix1(h_cylinder, size_edge_candidate, edge_candidate, point_all, edge, r_cylinder);
...
tform_spot_all = cell(size_sim,size_spot);
%%%%%%%%%%%%%%%%%%%%%
%%% 连杆坐标系定义 %%%
%%%%%%%%%%%%%%%%%%%%%
for idx_config = 1:size_sim
    ...
    [spot, tform_spot_all] = getSpotFrameAll(size_spot, position_helix, tform_link, position_link, r_cylinder, idx_config, tform_spot_all); %生成传感器点相对于世界坐标系的齐次变换矩阵
end
...
for idx_config = 1:size_sim %当前关节角配置
    ...
    tform_spot_config = tform_spot_all(idx_config,:);
    ...
    for idx_point_target = 1:size_point_target
        ...
        for idx_spot = 1:size_spot
            % tform_spot_current = tform_spot_all{idx_config,idx_spot};
            tform_spot_current = tform_spot_config{1,idx_spot};
    ...
    plotFoV(h_cone, fov_horizontal, tform_spot_config, size_spot);
