...
idx_config = 1; %当前关节角配置
   ...
   idx_link = 1;
        link_current = findobj('DisplayName',link_name(idx_link));
        ...
        [phi,paths] = heatMethod(X,F,pstart,pend);
        ...
        show(my_robot,config);
        link_current = findobj('DisplayName',link_name(idx_link));
        vertices_link_current = link_current.Vertices; %当前连杆的mesh坐标信息
        % hold on
        % scatter3(vertices_link_current(:,1),vertices_link_current(:,2),vertices_link_current(:,3),2,"magenta","filled")
