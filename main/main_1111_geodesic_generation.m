...
for index_facet = 1:size_facets% 遍历各起点（各三角面片形心点）
    ...
    for index_angle = 1:size_angle% 遍历各初始角度
        ...
        while length_sum < length_desired
            ...
            % % 排除曲率突变（阈值已增加）(由0.05修改为0.005)
            % if round >1
            %     if dot(faceNormal(TR,indices_facets_seq(round)),faceNormal(TR,indices_facets_seq(round-1)))<=0.005
            %         flag = 1;
            %         break
            %     end
            % end
