function phi = getGeodesicDistanceHeat(X,F,pstart)
...
%%% 离散几何算子 %%%
%%%%%%%%%%%%%%%%%%%
I = zeros(1,3*m); J = zeros(1,3*m); V = zeros(3,3*m); % indexes to build the sparse matrices
for i=1:3
...
I = zeros(1,6*m); J = zeros(1,6*m); V = zeros(1,6*m); % indexes to build the sparse matrices
Ia = zeros(1,3*m); Va = zeros(1,3*m); % area of vertices
...
for i=1:3
    ...
    % update the index listing
    I(1+(i-1)*2*m:i*2*m) = [F(s,:), F(t,:)];
    J(1+(i-1)*2*m:i*2*m) = [F(t,:), F(s,:)];
    V(1+(i-1)*2*m:i*2*m) = [ctheta, ctheta];
