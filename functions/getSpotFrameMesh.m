function [spot, tform_sensor_all] = getSpotFrameMesh(size_sensor, point_path, x_point_path, y_point_path, z_point_path, tform, position, r_cylinder)
% 生成螺旋线均分点（各传感器坐标系原点）
% spot = interparc(size_sensor,position_helix(1,:),position_helix(2,:),position_helix(3,:),'spline');
spot = getUniformPoints(size_sensor, point_path', size(point_path,2)); %自定义函数
spot = spot';
